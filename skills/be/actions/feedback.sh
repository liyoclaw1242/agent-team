#!/usr/bin/env bash
# feedback.sh — BE Mode C feedback. Mirrors fe/actions/feedback.sh.
#
# Usage:
#   feedback.sh
#       --issue N
...
# Validate header — must be exactly "## Technical Feedback from be"
first_line=$(grep -v '^$' "$FB_FILE" | head -n1)
expected="## Technical Feedback from be"
...
"$ROUTE_SH" "$ISSUE_N" "arch" \
  --repo "$REPO" \
  --agent-id "$AGENT_ID" \
  --reason "Mode C feedback from be; awaiting arch-feedback decision" \
  >/dev/null \
