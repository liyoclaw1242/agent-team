#!/usr/bin/env bash
# setup.sh — claim an FE issue, create the working branch, write the start
# journal entry. The agent calls this once per task at the start of work.
#
# Usage:
#   setup.sh
#       --issue N
#       --slug SLUG                    short branch slug, lowercase a-z 0-9 -
#       [--repo OWNER/REPO]
#       [--agent-id ID]
#
# Output (stdout):
...
#
# Side effects:
#   - claims.sh acquires the issue (status:ready → status:in-progress)
#   - setup-branch.sh creates fe/issue-N-SLUG branch
#   - write-journal.sh records the claim event
...
# 1. Claim the issue (atomic; refuses if already claimed)
"$CLAIMS_SH" "$ISSUE_N" \
  --repo "$REPO" \
  --agent-id "$AGENT_ID" \
  --note "fe starting work" \
  || { echo "failed to claim issue #$ISSUE_N" >&2; exit 3; }

# 2. Create working branch
branch=$("$SETUP_BRANCH_SH" fe "$ISSUE_N" "$SLUG") \
